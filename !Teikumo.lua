...
function on_kill_say(event)
    ...
    entity = engine.get_player_for_user_id(event:get_int("userid", 1))

    if kill_say:get_value() then
        if attacker == local_player and entity ~= local_player then
            engine.execute_client_cmd("say Isonami. via Teiku.moe.")
        end
    end
end
client.register_callback("fire_game_event", on_kill_say)
